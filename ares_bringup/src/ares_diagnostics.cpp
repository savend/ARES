...
#define HARDWARE_VERSION "2021.04.06"
#define FIRMWARE_VERSION_MAJOR_NUMBER 1
#define FIRMWARE_VERSION_MINOR_NUMBER 2

ros::Publisher ares_version_info_pub;
ros::Publisher ares_diagnostics_pub;
...
void sensorStateMsgCallback(const ares_msgs::SensorState::ConstPtr &msg)
{
  if (msg->battery > 11.0)
  ...
  if (msg->button == ares_msgs::SensorState::BUTTON0)
    setButtonDiagnosis(diagnostic_msgs::DiagnosticStatus::OK, "BUTTON 0 IS PUSHED");
  else if (msg->button == ares_msgs::SensorState::BUTTON1)
...
void firmwareVersionMsgCallback(const ares_msgs::VersionInfo::ConstPtr &msg)
{
  ...
  ares_msgs::VersionInfo version;

  version.software = SOFTWARE_VERSION;
  version.hardware = HARDWARE_VERSION;
  version.firmware = msg->firmware;

  ares_version_info_pub.publish(version);
}

void msgPub()
{
  diagnostic_msgs::DiagnosticArray ares_diagnostics;

  ares_diagnostics.header.stamp = ros::Time::now();

  ares_diagnostics.status.clear();
  ares_diagnostics.status.push_back(imu_state);
  ares_diagnostics.status.push_back(motor_state);
  ares_diagnostics.status.push_back(LDS_state);
  ares_diagnostics.status.push_back(battery_state);
  ares_diagnostics.status.push_back(button_state);

  ares_diagnostics_pub.publish(ares_diagnostics);
}

int main(int argc, char **argv)
{
  ros::init(argc, argv, "ares_diagnostic");
  ros::NodeHandle nh;

  ares_diagnostics_pub  = nh.advertise<diagnostic_msgs::DiagnosticArray>("diagnostics", 10);
  ares_version_info_pub = nh.advertise<ares_msgs::VersionInfo>("version_info", 10);

  ros::Subscriber imu         = nh.subscribe("imu", 10, imuMsgCallback);
  ros::Subscriber lds         = nh.subscribe("scan", 10, LDSMsgCallback);
  ros::Subscriber ares_sensor  = nh.subscribe("sensor_state", 10, sensorStateMsgCallback);
